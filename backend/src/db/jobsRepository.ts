...
export const jobsRepository = {
    ...
    async createJob(url: string, language: string = 'en', normalizedUrl?: string): Promise<Job> {
        try {
            ...
            if (error) throw new Error(`Database error creating job: ${error.message}`);
    ...
    async getJob(jobId: string): Promise<Job> {
        try {
            ...
            if (error) throw new Error(`Database error retrieving job: ${error.message}`);
    ...
    async updateJobStatus(jobId: string, status: JobStatus, details?: any): Promise<Job> {
        try {
            ...
            if (error) throw new Error(`Database error updating job status: ${error.message}`);
            if (!data) throw new Error(`Job with ID ${jobId} not found for status update`);
    ...
    async updateJobAsFailed(jobId: string, errorMessage: string): Promise<Job> {
        ...
        try {
            ...
            if (error) throw new Error(`Database error marking job as failed: ${error.message}`);
            if (!data) throw new Error(`Job with ID ${jobId} not found for failure update`);
    ...
    async updateJobAsComplete(jobId: string, results: AnalysisResults): Promise<Job> {
        ...
        try {
            ...
            if (error) throw new Error(`Database error marking job as complete: ${error.message}`);
            if (!data) throw new Error(`Job with ID ${jobId} not found for completion update`);
    ...
    async updateJobDetailsInDb(jobId: string, details: JobDetails): Promise<Job> {
        try {
            ...
            if (error) throw new Error(`Database error updating job details: ${error.message}`);
            if (!data) throw new Error(`Job with ID ${jobId} not found for details update`);
    ...
    async saveExtractedArticleContent(
        ...
        try {
            ...
            if (articleData.article_preview_image_url) {
                articleData.article_preview_image_url = getProxiedImageUrl(articleData.article_preview_image_url);
            }
            ...
            if (error) throw new Error(`Database error saving article content: ${error.message}`);
            if (!data) throw new Error(`Job with ID ${jobId} not found for article content update`);
     ...
     */
    async getJobTitleAndText(jobId: string): Promise<{ article_title: string; article_text: string }> {
        try {
            const { data, error } = await supabase
                .from('jobs')
            ...
            if (error) throw new Error(`Database error retrieving job content: ${error.message}`);
    ...
    async getJobHistory(limit = 50): Promise<any[]> {
        try {
            ...
            if (error) throw new Error(`Database error retrieving job history: ${error.message}`);
            return data || [];
        } catch (error) {
     ...
     */
    async findCompletedJobByNormalizedUrlAndLanguage(normalizedUrl: string, language: string): Promise<Job | null> {
        try {
            const { data, error } = await supabase
                .from('jobs')
                .select('*')
            ...
            if (error) throw new Error(`Database error finding job: ${error.message}`);
